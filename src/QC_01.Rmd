...
The thresholds set automatically are too relaxed. We will set stricter thresholds, but still
being permissive as this is the first round. 

We will also use the previous knowledge acquired working with similar datasets ( with younguer animals)

umi: 2000 min # this is the lowest value after the 2 QC rounds in the young dataset
detected; 900 min # the lowest value was 1000 genes detectect in the young datset
mit: 15 max 

The upper thresholds will be kept the same. we will also take in consideration the detected genes/umi counts ratio,
that filter out cells with relatively high umi counts but very few detected
genes ( lots of copies from the same genes).
...
# add label 'discard' to all cells to discard
sce$discard <- sce$outlier | sce$outlier_ratio | discard_mt | discard_umi | discard_detected
# save the object with all cells but with outlier/discard parametres saved
if (!file.exists(here( "processed", project, "sce_preliminary.RDS"))) {
