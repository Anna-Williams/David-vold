...
identified as dead cells or the erythrocytes ( that have expression of few genes)
...
# the microglia clusters will be the top differences, but this is not 
  ...
  filter(Proportion > ko_pct | Proportion < (100-ko_pct) ) %>% 
  .$cluster
```
...
abundant (greater than `r ko_pct` %) in the KO or WT, these are `r gt_ko_pct`
...
```{r filterout}
sce$filter_out <- sce$clusters_named %in% c( high_mt_clusters, low_umi_clusters, doublet) & !(sce$clusters_named %in% difference_KO_WT)
...
This would filter `r filter_out` cells, leaving an object with
