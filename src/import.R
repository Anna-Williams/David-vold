...
#cellcalling has been done by CellRanger. In Cellranger version 6 the cellcalling
#is done with a very similar algorithm to dropletutilities one.
...
samples <- dir(here("data/CellRanger/"))
matrices <- here("data/CellRanger/", samples, "outs/filtered_feature_bc_matrix")
metadata <- read.csv(here("data/metadata_scRNAseq.csv"))
metadata <- metadata[metadata$age == "very_old", ]
...
# Save the object
saveRDS(sce, here("processed", "sce_vold.RDS"))
